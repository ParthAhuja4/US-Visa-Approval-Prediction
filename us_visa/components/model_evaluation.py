...
class ModelEvaluation:
    ...
    def get_best_model(self) -> Optional[USvisaEstimator]:
        try:
            usvisa_estimator = USvisaEstimator(
                ...
                model_filename=self.model_eval_config.hf_model_filename,
            )
    ...
    def evaluate_model(self) -> EvaluateModelResponse:
        ...
        try:
            ...
            best_model_f1_score: Optional[float] = None  # ← explicitly typed
            best_model = self.get_best_model()
            if best_model is not None:
                y_hat_best_model = best_model.predict(x)
                y_hat_best_model = pd.Series(y_hat_best_model).replace(
                    TargetValueMapping()._asdict()
                )
                best_model_f1_score = float(
                    f1_score(y.astype(int), y_hat_best_model.astype(int))
                )  # ← cast here

            tmp_best_model_score: float = (
                0.0 if best_model_f1_score is None else best_model_f1_score
            )
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifact:
        ...
        try:
            ...
            hf_model_path = self.model_eval_config.hf_model_filename
